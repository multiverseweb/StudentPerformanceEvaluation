...
def visualize_marks():
    # Get marks for the current user
    ...
    for student in students_data:
        for subject in subjects:
            if subject in students_data[student]['marks']:
                total_marks[subject] += int(students_data[student]['marks'][subject])

    average_scores = [total_marks[subject] / student_count for subject in subjects]

    # Create bar positions
    x = range(len(subjects))

    # Create the bar graph
    plt.bar(x, scores, width=0.4, color='#eb9b34', label=f'{current_user} Marks', align='center')
    plt.bar([p + 0.4 for p in x], average_scores, width=0.4, color='#02b087', label='Average Marks', align='center')

    # Add labels and title
    plt.xlabel('Subjects')
    plt.ylabel('Marks')
    plt.title(f'Marks Comparison for {current_user}')
    plt.xticks([p + 0.2 for p in x], subjects)  # Set the x ticks to be in the middle of the bars
    plt.legend()
